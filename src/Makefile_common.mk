...
VER_URL = 0.8.1
DIR_URL = $(DEP_DIR)/uriparser-$(VER_URL)
INC_URL = -isystem$(DIR_URL)/include/uriparser
LIB_URL_STATIC = $(DIR_URL)/.libs/liburiparser.a
LIB_URL_SHARED = $(DIR_URL)/.libs/liburiparser.so
VER_ONIG = 5.9.5
...
LIB_MOZ_STATIC = $(DIR_MOZ)/js/src/build/js/src/libjs_static.a
LIB_MOZ_SHARED = $(DIR_MOZ)/js/src/build/js/src/libmozjs-$(shell echo $(VER_MOZ)|cut -d'.' -f1).so
VER_NSPR = 4.10.9
...
DIRS = $(DIR_PICOEV) $(DIR_H3) $(DIR_CLOG) $(DIR_TADL) $(DIR_ONIG) $(DIR_PG) $(DIR_MOZ) $(DIR_NSPR) $(DIR_Z) $(DIR_DST) $(DIR_URL)
INCS = $(INC_PICOEV) $(INC_H3) $(INC_CLOG) $(INC_TADL) $(INC_ONIG) $(INC_PG) $(INC_MOZ) $(INC_NSPR) $(INC_Z) $(INC_URL)
ifeq ($(MAKE_STATIC_LIBS),yes)
DEPS = $(LIB_PICOEV_STATIC) $(LIB_H3_STATIC) $(LIB_CLOG_STATIC) $(LIB_TADL_STATIC) $(LIB_ONIG_STATIC) $(LIB_PG_STATIC) $(LIB_MOZ_STATIC) $(LIB_NSPR_STATIC) $(LIB_Z_STATIC) $(LIB_URL_STATIC)
LIBS = $(LIB_PICOEV_STATIC) $(LIB_H3_STATIC) $(LIB_CLOG_STATIC) $(LIB_TADL_STATIC) $(LIB_ONIG_STATIC) $(LIB_PG_STATIC) $(LIB_MOZ_STATIC) $(LIB_NSPR_STATIC) $(LIB_Z_STATIC) $(LIB_URL_STATIC)
else
DEPS = $(LIB_PICOEV_SHARED) $(LIB_H3_SHARED) $(LIB_CLOG_SHARED) $(LIB_TADL_SHARED) $(LIB_ONIG_SHARED) $(LIB_PG_SHARED) $(LIB_MOZ_SHARED) $(LIB_NSPR_SHARED) $(LIB_Z_SHARED) $(LIB_URL_SHARED)
LIBS = 	$(call sharedLibFileToParam, $(LIB_PICOEV_SHARED)) \
		...
		$(call sharedLibFileToParam, $(LIB_URL_SHARED))
endif
